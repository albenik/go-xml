...
func (s *Scope) JoinScope(inner *Scope) *Scope {
	return &Scope{append(s.ns, inner.ns...)}
}
...
func (s *Scope) Prefix(name xml.Name) (qname string) {
	switch name.Space {
	case "":
		return name.Local
	case xmlLangURI:
...
func (s *Scope) pushNS(tag xml.StartElement) []xml.Attr {
	var ns []xml.Name
	...
	if len(ns) > 0 {
		s.ns = append(s.ns, ns...)
		// Ensure that future additions to the s create
		// a new backing array. This prevents the s from
